...
handler_t acceptor_run(void * e, void * s) {
    ...
    int32_t nfd = -1;
    socket_t * listen_socket = (socket_t *)s;
    if (listen_socket->status == ISWRITE) { //表示处于监听
        if((nfd = socket_accept(listen_socket->fd, (struct sockaddr*)&client_address, &len)) == -1) {  
